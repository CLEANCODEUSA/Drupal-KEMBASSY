...
namespace Drupal\cms_custom_module_openweather;
...
class WeatherService {
     ...
     * @var Drupal\openweather
     */
    public static $basegeoUri = 'http://api.geonames.org/';
    //public static $basegeoUri = 'https://ws.geonames.org/';
    ...
    public function createRequest($options) {
        if ($_GET['zip_code']) {
            $id_zip = $_GET['zip_code'];
        }
        $query = [];
        $appid_config = \Drupal::config('cms_custom_module_openweather.settings')->get('appid');
        $query['appid'] = Html::escape($appid_config);
        $query['cnt'] = \Drupal::config('cms_custom_module_openweather.settings')->get('count');
        $input_data = \Drupal::config('cms_custom_module_openweather.settings')->get('input_value');
     ...
     */
    //$options['display_type'] = 'current_details';
    ...
    public function getCurrentWeatherInformation($output, $config) {
        ...
        $build[] = [
            ...
            '#attached' => array(
                'library' => array(
                    'cms_custom_module_openweather/openweather_theme',
                ),
            ),
    ...
    public function getHourlyForecastWeatherInformation($output, $config) {
        ...
        $build[] = [
            ...
            '#attached' => array(
                'library' => array(
                    'cms_custom_module_openweather/openweatherhourlyforecast_theme',
                ),
            ),
            '#cache' => array('max-age' => 0),
        ];
    ...
    public function getDailyForecastWeatherInformation($output, $config) {
        ...
        $build[] = [
            ...
            '#attached' => array(
                'library' => array(
                    'cms_custom_module_openweather/openweatherdailyforecast_theme',
                ),
            ),
            '#cache' => array('max-age' => 0),
        ];
